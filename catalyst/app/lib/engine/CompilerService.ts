...
export class CompilerService {
    ...
    switch (target) {
      case TargetModel.CLAUDE_3_5_SONNET:
        ({ formattedPrompt, systemInstruction } =
          this.compileForClaude(metadata));
        break;
      ...
      case TargetModel.GEMINI_1_5_PRO:
        ({ formattedPrompt, systemInstruction } =
          this.compileForGemini(metadata));
        break;
      default:
   ...
   */
  private compileForClaude(meta: DeconstructedPrompt): {
...
Please provide your response below:`;
   ...
   */
  private compileForGPT(meta: DeconstructedPrompt): {
    formattedPrompt: string;
    systemInstruction: string;
  } {
...
${meta.assets.length > 0 ? `NOTE: Consider the visual/data evidence in the provided assets.` : ""}`;
   ...
   */
  private compileForGemini(meta: DeconstructedPrompt): {
    formattedPrompt: string;
    systemInstruction: string;
  } {
