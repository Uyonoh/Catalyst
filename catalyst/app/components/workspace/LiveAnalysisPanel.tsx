  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-cyan-400 self-start sm:self-auto">
          <span className="relative flex h-2 w-2">
            ...
            <span
              className={`relative inline-flex rounded-full h-2 w-2 ${isLoading ? "bg-cyan-500" : "bg-slate-600"}`}
            />
          </span>
          {isLoading
            ? "Real-time Parsing..."
      ...
      <div className="flex flex-col gap-6 flex-1">
        <div
          className={`flex-col gap-6 ${showDetails ? "flex" : "hidden md:flex"} animate-fadeIn`}
        >
          {/* Confidence Score */}
        ...
        <div className="flex-1 flex flex-col space-y-2 min-h-[140px]">
          ...
          <div
            className={`code-preview flex-1 rounded-lg p-4 overflow-hidden relative group bg-black/20 border transition-all duration-300 ${isLoading ? "border-cyan-500/50 shadow-[0_0_15px_rgba(6,182,212,0.2)]" : "border-white/5"}`}
          >
            ...
            <code
              className={`text-sm font-mono leading-relaxed block text-slate-300 overflow-x-auto transition-opacity duration-300 ${isLoading ? "opacity-40" : "opacity-100"}`}
            >
              {result ? (
