...
export default function TokenAnalyticsCard({
  ...
  const remaining = isEnterprise ? Infinity : Math.max(0, limit - dailyTokensUsed);
  const percentage = isEnterprise ? 0 : Math.min(100, Math.round((dailyTokensUsed / limit) * 100));
  ...
  let limitLabel = isEnterprise ? "Unlimited" : `${dailyTokensUsed} / ${limit} Daily`;
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 mb-12 flex flex-col relative z-10 animate-slideDown overflow-hidden border border-white/5 shadow-2xl" style={{ animationDelay: "0.15s", animationFillMode: "both" }}>
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute right-0 bottom-7 w-64 p-3 bg-slate-950/95 border border-white/10 rounded-lg text-[11px] text-slate-300 leading-relaxed shadow-xl backdrop-blur-md z-30 transition-all">
              Optimizing prompts consumes daily tokens based on model intelligence and modalities. Free tier grants 50 tokens daily. Pro Tier expands quota to 200 tokens. Quota resets at 00:00 UTC.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-6 items-center justify-between mb-8 z-10 pb-6 border-b border-white/5">
            ...
            <div className="absolute flex flex-col items-center justify-center text-center">
              {isEnterprise ? (
                <>
                  ...
                  <span className="text-[10px] text-purple-400 font-bold uppercase tracking-wider mt-1">Unlimited</span>
                </>
              ) : (
                <>
                  ...
                  <span className="text-[9px] text-slate-400 font-semibold uppercase tracking-wider mt-1">Remaining</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="sm:col-span-7 flex flex-col gap-3 justify-center">
          <div className="flex items-center justify-between">
            <span className="text-xs text-slate-400 font-medium">Catalyst Plan</span>
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
              plan === "enterprise" ? "bg-purple-500/20 text-purple-300 border border-purple-500/30" :
              plan === "pro" ? "bg-cyan-500/20 text-cyan-300 border border-cyan-500/30" :
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-slate-400 font-medium">Daily Tokens Spent</span>
          ...
          {!isEnterprise && (
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-400 font-medium">Next Reset Countdown</span>
      ...
      <div className="z-10 mt-2">
        <h4 className="text-white text-xs font-bold uppercase tracking-wider mb-3 flex items-center justify-between">
          ...
          <a href="/settings/subscriptions#usage-stats" className="text-[10px] text-slate-500 hover:text-cyan-400 flex items-center transition-colors">
        ...
        {recentLogs.length === 0 ? (
          ...
          <div className="flex flex-col gap-2 max-h-[170px] overflow-y-auto pr-1 dropdown-scroll">
            {recentLogs.map((log) => {
              ...
              return (
                <div key={log.id} className="flex items-center justify-between p-2 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all group">
                  <div className="flex items-center gap-2.5">
                    <div className={`p-1.5 rounded-lg border ${theme.bgClass} flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform`}>
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[8px] font-bold uppercase tracking-wider px-1 py-0.25 bg-white/5 text-slate-400 rounded leading-none">{log.mode}</span>
                      </div>
                      <span className="text-[9px] text-slate-500 font-medium font-mono">{date}</span>
                    </div>
                  </div>
                  <span className={`text-xs font-bold font-mono ${theme.colorClass}`}>
                    -{log.cost}
                  </span>
                </div>
              );
