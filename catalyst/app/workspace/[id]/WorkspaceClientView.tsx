...
export default function WorkspaceClientView({
  ...
  const [editVisibility, setEditVisibility] = useState<"private" | "public" | "community">(workspace.visibility as any);
  ...
  const canAddPrompts = isWorkspaceOwner || workspace.visibility === "community";
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              <span className={`flex items-center gap-1.5 px-3 py-1 text-[10px] font-black uppercase tracking-wider rounded-lg border ${getVisibilityBadgeStyle(workspace.visibility)}`}>
            ...
            <p className="text-slate-400 text-sm md:text-base mt-2 leading-relaxed max-w-2xl">
              {workspace.description || "No description provided for this engineering workspace."}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {canAddPrompts && (
              <Link
                ...
                className="px-5 py-2.5 text-xs font-black text-white bg-gradient-to-r from-cyan-500 to-purple-600 rounded-xl shadow-neon hover:shadow-neon-strong transition-all transform hover:-translate-y-0.5 active:scale-95 flex items-center gap-1.5 cursor-pointer"
              >
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <h2 className="text-white font-bold text-lg flex items-center gap-2 self-start">
            Workspace Prompts
            <span className="text-xs font-normal text-slate-400">({filteredPrompts.length})</span>
          </h2>
        ...
        {promptToDelete && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/75 backdrop-blur-sm" onClick={() => setPromptToDelete(null)} />
            <div className="relative z-10 w-full max-w-sm rounded-xl border border-rose-500/20 bg-[#0c1520] p-6 shadow-2xl animate-in fade-in zoom-in-95">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-white font-bold text-sm">Delete Prompt</h3>
                  <p className="text-slate-400 text-xs mt-1 leading-relaxed">
                    Are you sure you want to delete <span className="text-white font-semibold">"{promptToDelete.title}"</span>? 
                    This action cannot be undone and will permanently remove this prompt.
                  </p>
                </div>
              </div>
              {deleteError && (
                <span className="text-[10px] text-rose-400 font-medium block mb-2">{deleteError}</span>
              )}
              <div className="flex items-center justify-end gap-3">
                ...
                >
                  {isDeletingPrompt ? <Loader2 className="size-3 animate-spin" /> : <Trash2 className="size-3" />}
                  Delete
                </button>
              </div>
            </div>
        ...
        {filteredPrompts.length === 0 ? (
          <div className="py-16 text-center border border-dashed border-white/5 rounded-2xl glass-panel">
            ...
            <h3 className="text-white font-bold text-sm mb-1">No prompts found</h3>
            <p className="text-slate-500 text-xs max-w-xs mx-auto">
              {searchQuery ? "No prompts match your current search query." : "There are no engineering prompts in this workspace yet."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="border-t border-white/5 pt-3 mt-4 flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
                      <User className="size-3" />
                      Added by {currentUserId === prompt.user_id ? "You" : prompt.authorName}
                    </span>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      >
                        {copiedId === prompt.id ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
                      </button>
