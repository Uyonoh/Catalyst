...
export default function ModelSelector() {
 const {
  ...
  let { models } = useCatalog();

  const imageModels = ["pollinations", "huggingface"];
  models = models.filter(model => imageModels.includes(model.slug));
  ...
  useEffect(() => {
    ...
    // Select model from new models list
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`hidden sm:flex items-center gap-2 md:gap-3 px-3 md:px-4 py-3 rounded-lg bg-[#101922] border ${getModelColor(selectedModel.color)} text-sm font-bold transition-all focus:outline-none active:scale-95`}
      ...
      <div className="sm:hidden flex items-center gap-2">
        <button
          onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          className={`flex items-center gap-1 px-2.5 py-3 rounded-lg bg-[#101922] border ${getModelColor(selectedModel.color).split(" shadow")[0]} text-xs font-bold transition-all active:scale-95`}
