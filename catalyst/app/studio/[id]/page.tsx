    ...
    async function fetchPrompt() {
      ...
      try {
        ...
        if (data) {
          ...
          setEditedText(data.content || data.snippet || "");
          setRawIntent(data.raw_intent || "No raw intent available");
          setIsAuthor(true);

          if (data.target_model) {
            const model = MODELS.find(
              (m) => m.id === data.target_model || m.name === data.target_model,
            );
            if (model) {
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex flex-col gap-1">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tight text-white flex items-center gap-3">
                Prompt Editor
                {isLoading && (
                  <span className="material-symbols-outlined animate-spin-slow text-cyan-400 text-2xl">
                    progress_activity
                  </span>
                )}
              </h1>
