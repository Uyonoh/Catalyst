...
export default function PromptEditorView({ id, initialData }: PromptEditorViewProps) {
  const router = useRouter();

  const [title, setTitle] = useState(initialData.title || "Untitled");
  const [editedText, setEditedText] = useState(initialData.content || "");
  const [rawIntent, setRawIntent] = useState(initialData.raw_intent || "No raw intent available");
  const [isAuthor, setIsAuthor] = useState(true);
  
  const initialModel = MODELS.find(
    (m) => m.id === initialData.target_model || m.name === initialData.target_model
  ) || MODELS[0];
