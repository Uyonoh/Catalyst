    ...
    return (
        <>
            ...
            <GlassPanel className="rounded-xl p-4 flex flex-col gap-4 h-full">
                <div className="flex flex-col gap-2 overflow-y-auto pr-2" style={{ maxHeight: '240px' }}>
                    {MODELS.map((model) => (
                        <label
                            key={model.id}
                        ...
                        >
                            ...
                            <span className={`ml-auto text-xs px-2 py-0.5 rounded ${selectedModel === model.id ? 'text-cyan-200 bg-cyan-900/40' : 'text-slate-400 bg-slate-800/40'}`}>
                                {model.type}
                            </span>
                        </label>
                    ))}
